...
const loadDisplaySetIntoViewport = (data, templateData) => {
    ...
    // if (OHIF.viewer.data.loadedSeriesData && OHIF.viewer.data.loadedSeriesData instanceof Array) {
...
Template.imageViewerViewport.onRendered(function() {
    ...
    const { layoutManager } = OHIF.viewerbase;
    const numberofViewports = layoutManager.getNumberOfViewports();
    if(numberofViewports > 1){
        for(i = 1; i < numberofViewports; i++){
            document.getElementsByClassName("roundedButtonGroup")[i].style.display = "none";
        }
    }
});
...
function drawCoordinates(x,y){  
    var ctx = document.getElementsByClassName("markerPenCanvas").getContext("2d");
...
function markPoint(event){
    if(Session.get('draw')){
        var offset = $('.markerPenCanvas').offset();
...
Template.imageViewerViewport.helpers({
    leftSidebarToggleButtonData() {
        return {
            toggleable: true,
            key: 'leftSidebar',
            value: OHIF.viewerbase.state,
            options: [{
                value: 'studies',
                svgLink: '/images/Series-Toggle.svg#icon',
                svgWidth: 25,
                svgHeight: 25,
                bottomLabel: 'Series'
            }]
