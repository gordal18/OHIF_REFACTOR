...
Template.toolbarSectionButton.helpers({
    ...
    svgLink() {
        ...
        const activeToolId = OHIF.viewerbase.toolManager.getActiveTool();
        var svgLink;
        if(instance.data.id === 'draw'){
            if(Session.get('drawTool'))
                svgLink = instance.getActiveToolSubProperty('svgLink_on', 'draw');
            else
                svgLink = instance.getActiveToolSubProperty('svgLink_over', 'draw');
            return _.isFunction(svgLink) ? svgLink() : svgLink;
        }
        else{
            if(!Session.get('drawTool')){
                if(instance.data.id === activeToolId){
                    svgLink = instance.getActiveToolSubProperty('svgLink_on', activeToolId);
                }
                else{
